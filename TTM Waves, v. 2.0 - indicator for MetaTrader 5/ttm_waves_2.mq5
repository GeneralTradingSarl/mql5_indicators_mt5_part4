...
#property indicator_label1  "TTM waves"
#property indicator_type1   DRAW_FILLING
#property indicator_color1  clrLime,clrOrange
#property indicator_label2  "TTM waves"
...
int OnInit()
{
   SetIndexBuffer(0,wfu,INDICATOR_DATA); 
   SetIndexBuffer(1,wfd,INDICATOR_DATA); 
   SetIndexBuffer(2,wave,INDICATOR_DATA); 
   SetIndexBuffer(3,wavec,INDICATOR_COLOR_INDEX); 
   ...
   for (int i=(int)MathMax(prev_calculated-1,1); i<rates_total; i++)
   {
      ...
      double xma3  = iLinr(price,Period3,i,2);
         ...
         work[i][_diff3] = iLinr(xma3-price,SmoothPeriod,i,5);
               ...
               wfu[i]  = 0;
               wfd[i]  = 0;
                  if (wave[i]>0 && wave[i]>wave[i-1]) wfu[i] = wave[i];
                  if (wave[i]<0 && wave[i]<wave[i-1]) wfu[i] = wave[i];

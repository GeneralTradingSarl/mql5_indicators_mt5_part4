...
#property description "Три осциллятора TriX с разных таймфреймов с одинаковыми параметрами на одном графике."
...
#define INDICATOR_NAME "TriX"  // константа для имени индикатора
...
#property indicator_label1  "TriX 1"
//+----------------------------------------------+
...
//--- отображение метки индикатора
#property indicator_label2  "TriX 2"
//+----------------------------------------------+
...
//--- отображение метки индикатора
#property indicator_label3  "TriX 3"
//+-------------------------------------+
...
//--- инициализация переменных 
   min_rates_total=2;
//--- получение хендла индикатора TriX 1
   Ind1_Handle=iTriX(Symbol(),TimeFrame1,TriXPeriod,TriXPrice);
   if(Ind1_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора TriX 1");
      return(INIT_FAILED);
     }
//--- получение хендла индикатора TriX 2
   Ind2_Handle=iTriX(Symbol(),TimeFrame2,TriXPeriod,TriXPrice);
   if(Ind2_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора TriX 2");
      return(INIT_FAILED);
     }
//--- получение хендла индикатора TriX 3
   Ind3_Handle=iTriX(Symbol(),TimeFrame3,TriXPeriod,TriXPrice);
   if(Ind3_Handle==INVALID_HANDLE)
     {
      Print(" Не удалось получить хендл индикатора TriX 3");
      return(INIT_FAILED);
     }
...
//--- создание имени для отображения в отдельном подокне и во всплывающей подсказке
   ...
   StringConcatenate(shortname,INDICATOR_NAME,"(",
                     GetStringTimeframe(TimeFrame1)," ,",GetStringTimeframe(TimeFrame2)," ,",GetStringTimeframe(TimeFrame3)
                     ," ,",TriXPeriod," ,",GetStringPrice(TriXPrice),")");
//--- 
